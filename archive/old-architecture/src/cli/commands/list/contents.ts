...
import { resolveRulepackPath } from '../../../utils/rulepackResolver';
...
export async function listRulePackContents(
  ...
  if (rulepackName.includes('/') || rulepackName.includes('\\')) {
    // Full path provided
    rulepackPath = resolveRulepackPath(rulepackName);
    displayName = rulepackName.split(/[/\\]/).pop() || rulepackName;
  } else {
    // Just filename provided - prepend rulepacks/ and resolve
    rulepackPath = resolveRulepackPath(join('rulepacks', rulepackName));
