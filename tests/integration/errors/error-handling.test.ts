...
describe('Error Handling Integration', () => {
  ...
  test('handles missing files gracefully', async () => {
    await expect(
      applyRulesToDataOrStream('missing-file.json', 'rulepacks/pii.yaml', false, false)
    ).rejects.toThrow();
  });
      ...
      const invalidRulepack = await fsHelpers.createTempFile('invalid: yaml: content', '.yaml');
      ...
      await expect(
      applyRulesToDataOrStream(tempFile, '/nonexistent/path/rulepack.yaml', false, false)
    ).rejects.toThrow();
  });
}); 